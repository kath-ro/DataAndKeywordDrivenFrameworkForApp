...
def write_result(excelObj, sheet, testResult, rowNo, colNo, errMsg = None, errPicPath=None):
    # 用例执行结束后，向excel中写入执行结果信息
    # 通过测试结果信息，判断成功为绿色，失败为红色
    colorDict = {"pass": "green", "faild": "red", "": None}
    # 测试用例 工作表和用例步骤工作表中都有执行时间和执行结果列，但映射的数据变量不同，所以需要根据测试结果列的字典对象，区分是哪一个工作表
    colsDict = {
    ...
    try:
        # 在测试用例、测试步骤或数据源工作表中，写入测试时间和测试结果
        if testResult == "":
            excelObj.writeCell(
                sheet, testResult,
                rowNo = rowNo, colsNo=colsDict[colNo][0], style=colorDict[testResult.lower()])
        else:
            excelObj.writeCellCurrentTime(
                sheet, rowNo=rowNo, colsNo=colsDict[colNo][0], style=colorDict[testResult.lower()])
        excelObj.writeCell(sheet, testResult,
                rowNo = rowNo, colsNo=colsDict[colNo][1], style=colorDict[testResult.lower()])
        if colNo == "caseStep":
            # 测试步骤执行失败，在测试步骤工作表中写入异常信息和截图路径
        ...
        print("写入excel出错\n错误信息:%s" %str(traceback.format_exc()))